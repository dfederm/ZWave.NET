<!--
  ...
  section: "4.51.13 Irrigation Valve Config Set Command"
  pages: 323-326
-->
...
This field MUST be set to 0 by a sending node and MUST be ignored by a receiving node. Master Valve (1 bit)

This field is used to indicate whether the valve to configure is the master valve or a zone valve.

The value 1 MUST indicate that the valve to configure is the master valve. The value 0 MUST indicate that the valve to configure is the zone valve corresponding to Valve ID field.
...
[The receiving node MUST issue an Irrigation System Status Report Command with the System Error](04.51.07-irrigation-system-status-report-command.md#4517-irrigation-system-status-report-command) Status indicating that a valve is reporting errors when the current crosses above this threshold.
...
[The receiving node MUST issue an Irrigation System Status Report Command with the System Error](04.51.07-irrigation-system-status-report-command.md#4517-irrigation-system-status-report-command) Status indicating that a valve is reporting errors when the current crosses below this threshold.
...
A receiving node having no support for flow sensor MAY ignore these fields. [If the receiving node supports a flow sensor, the node MUST issue an Irrigation System Status Report Command with the System Error Status indicating that a valve is reporting errors when the water flow](04.51.07-irrigation-system-status-report-command.md#4517-irrigation-system-status-report-command) crosses above this threshold.
...
This field MUST be ignored if the Master Valve field is set to 1.

[This field MUST be treated as a bit mask and MUST comply with Table 68, Irrigation Valve Config Set::Sensor Usage.](04.51.13-irrigation-valve-config-set-command.md#45113-irrigation-valve-config-set-command)
