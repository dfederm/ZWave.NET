<!--
  ...
  section: "4.84 Scene Actuator Configuration Command Class, version 1"
  pages: 472-476
-->
# 4.84 Scene Actuator Configuration Command Class, version 1
...
This field MUST specify the time it MUST take to reach the target level associated to the actual Scene ID. A receiving node always starts from current level. The dimming duration MUST be the same independently of the number of levels to be changed.
...
[This field MUST be encoded according to Table 7, Actuator Command Class Duration Set encoding.](../03-command-class-overview/03.07-common-fields-and-encoding.md#373-duration-encoding)
...
This field is used to specify the requested Scene ID settings. Values in the range 1..255 MUST indicate that the receiving node MUST return settings associated to the actual Scene ID..

The value 0 MUST indicate that the current active scene (if any).

## 4.84.4 Scene Actuator Configuration Report Command
...
[This field MUST be encoded according to Table 104.](04.84-scene-actuator-configuration-command-class-version-1.md#4844-scene-actuator-configuration-report-command) Table 104, Scene Actuator Configuration Report::Dimming Duration encoding
