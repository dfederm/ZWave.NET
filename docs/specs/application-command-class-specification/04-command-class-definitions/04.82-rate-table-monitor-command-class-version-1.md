<!--
  ...
  section: "4.82 Rate Table Monitor Command Class, version 1"
  pages: 460-471
-->
...
Number of rates supported (1..255). Parameter Set Supported Bit Mask 1, 2 (16 bits)
...
## 4.82.4 Rate Table Report Command
...
[Refer to description of fields under the Rate Table Set Command (section 4.81.1).](04.81-rate-table-configuration-command-class-version-1.md#4811-rate-table-set-command)
...
## 4.82.6 Rate Table Active Rate Report Command
...
The dataset requested parameter indicates which data is requested by the command. This field MUST [be encoded according to the Meter Datasets defined in [21].](04.117-window-covering-command-class-version-1.md#411710-window-covering-stop-level-change-command)
...
This value indicates how many report frames there are left, the value 0xFF means that the number of reports have not been calculated yet or that there is more than 255 reports to follow. Rate Parameter Set ID (8 bits)

The Rate Parameter Set ID addresses the wanted parameter set. The rate parameter set identifier MUST be a sequence starting from 1 to Rates Supported.

Dataset (24 bits)

The dataset parameter indicates which data is included in the report. This field MUST be encoded [according to the Meter Dataset defined in [21].](04.117-window-covering-command-class-version-1.md#411710-window-covering-stop-level-change-command)
...
The Current Scale is used to indicate the scale (unit) of the following value. This field MUST be encoded [according to the Meter Dataset and Scales defined in [21].](04.117-window-covering-command-class-version-1.md#411710-window-covering-stop-level-change-command)

Current Value (N * 32 bits)

The Current Value advertises a value corresponding to the Dataset Requested field of the Get Command. The field MUST be encoded as a 32-bit signed integer. The first byte (Value 1) MUST carry [most significant byte. Table 10, Signed field encoding (two’s complement representation) shows signed](../03-command-class-overview/03.07-common-fields-and-encoding.md#375-signed-encoding) decimal values together with their hexadecimal equivalents.
...
The dataset requested parameters indicate data requested. This field MUST be encoded according to [the Meter Dataset defined in [21].](04.117-window-covering-command-class-version-1.md#411710-window-covering-stop-level-change-command)
