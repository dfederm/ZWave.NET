<!--
  ...
  section: "4.79 Protection Command Class, version 2"
  pages: 448-454
-->
...
The Local Protection State field used to set the protection state of the device. Table 99, Protection Set::Local Protection State encoding
...
## 4.79.2 Protection Report Command
...
[For field description, refer to 4.79.1 Protection Set Command.](04.79-protection-command-class-version-2.md#4791-protection-set-command)
...
## 4.79.4 Protection Supported Report Command
...
[The list of all Local Protection States may be found in section 4.79.1. The two bytes MUST be](04.79-protection-command-class-version-2.md#4791-protection-set-command) interpreted as bit masks where byte 1 bit 0 represent Local Protection State 0, byte 1 bit 1 represent Protection State 1, byte 2 bit 0 represent Protection State 8 etc.

RF Protection State Byte (2 bytes)

[The list of all RF Protection States may be found in section 4.79.1. The two bytes MUST be interpreted](04.79-protection-command-class-version-2.md#4791-protection-set-command) as bit masks where byte 1 bit 0 represent RF Protection State 0, byte 1 bit 1 represent Protection State 1, byte 2 bit 0 represent Protection State 8 etc.
...
All of the Protection Command Class commands will be accepted and processed regardless of whether or not a node has exclusive control. Factory default setting of the NodeID for exclusive control MUST be set to 0. To reset the exclusive control state in a device an Exclusive Control Set Command with NodeID 0 as parameter MUST be send to the device.
...
[See description under the Protection Exclusive Control Set Command section 4.79.5.1.](04.79-protection-command-class-version-2.md#47951-protection-exclusive-control-set-command)
...
### 4.79.6.1 Protection Timeout Set Command
...
The timeout describes the time that a device MUST remain in RF Protection mode. Factory default setting for the Timeout parameter MUST be 0x00. This field MUST be encoded [according to Table 101.](04.79-protection-command-class-version-2.md#47961-protection-timeout-set-command)
...
### 4.79.6.3 Protection Timeout Report Command
...
[This field indicates the remaining timeout set in the Node. It MUST be encoded as described in Table 101](04.79-protection-command-class-version-2.md#47961-protection-timeout-set-command)
