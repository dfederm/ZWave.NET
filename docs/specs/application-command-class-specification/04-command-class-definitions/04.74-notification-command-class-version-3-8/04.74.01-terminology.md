...
An event only has a meaning in the moment it happens. An event does not indicate the value of a state [variable. An example is given in Figure 15](04.74.01-terminology.md#4741-terminology)

![Figure 15, Event notifications inform only about instantaneous situations](assets/img-10ce66b311.png)

A state variable may assume two or more states. Some state variables are returned to their idle state via a generic “State idle” Notification. For example, a controlling node receiving a {Smoke alarm::Smoke detected} Notification will consider that the state has not changed until it receives a {Smoke alarm::State idle(Smoke detected)} [Notification. An illustration is given in Figure 16.](04.74.01-terminology.md#4741-terminology)

![Figure 16, Binary state variable with generic “State idle” Notification](assets/img-a5a0d9d7be.png)

The “State idle” notification may also be used to return a multi-value state variable to its idle state. An [example is given inFigure 17.](04.74.01-terminology.md#4741-terminology)

![Figure 17, Multi-value state variable with generic “State idle” Notification](assets/img-de17561660.png)

[Some state variables use a specific Notification defined for each state change. Figure 18 shows an](04.74.01-terminology.md#4741-terminology) example of such a state variable.

![Figure 18, Binary state variable with specific idle state Notification](assets/img-fb25e9706c.png)
...
[An illustration is given in Figure 19, where the same Notifications with different parameters advertise](04.74.01-terminology.md#4741-terminology) the state of different state variables.

![Figure 19, State variable array defined by additional Event/State Parameters](assets/img-be382647c3.png)

[A complete list of Notifications (including state variables, event parameters, etc.) is given in [18].](../04.117-window-covering-command-class-version-1.md#411710-window-covering-stop-level-change-command)
